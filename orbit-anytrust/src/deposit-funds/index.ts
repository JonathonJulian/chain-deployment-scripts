...
async function main() {
  ...
  const L2_RPC_URL = process.env.ORBIT_CHAIN_RPC
  const L3_RPC_URL = process.env.PARENT_CHAIN_RPC
  const amount = process.env.AMOUNT
  console.log(L2_RPC_URL, L3_RPC_URL, amount)

  if (!privateKey || !L2_RPC_URL || !L3_RPC_URL || !amount) {
    const missingVars: string[] = []  // Explicitly define the type as string[]
    if (!privateKey) missingVars.push('PRIVATE_KEY')
    if (!L2_RPC_URL) missingVars.push('ORBIT_RPC_URL')
    if (!L3_RPC_URL) missingVars.push('PARENT_CHAIN_RPC_URL')
    if (!amount) missingVars.push('AMOUNT')
    throw new Error(`Required environment variable(s) not found: ${missingVars.join(', ')}`)
  }
